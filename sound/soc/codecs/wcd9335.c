	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if (1 == ucontrol->value.integer.value[0]) {
		pr_info("%s, enable!\n", __func__);
                regmap_update_bits(wcd9xxx->regmap,WCD9335_TEST_DEBUG_NPL_DLY_TEST_1,0x90, 0x10);
		msleep(10);
                regmap_update_bits(wcd9xxx->regmap,WCD9335_TEST_DEBUG_NPL_DLY_TEST_1,0x90, 0x90);
	} else {
		pr_info("%s, disable!\n", __func__);
                regmap_update_bits(wcd9xxx->regmap,WCD9335_TEST_DEBUG_NPL_DLY_TEST_1,0x90, 0x10);
		msleep(10);
